    ...
    return (
      <span className="inline-flex items-center ml-1" title="Read">
        <svg width="16" height="10" viewBox="0 0 16 10" fill="none">
          <path d="M1 5l3 3 5-6" stroke="#60a5fa" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M6 5l3 3 5-6" stroke="#60a5fa" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </span>
    );
  }
  ...
  return (
    <span className="inline-flex items-center ml-1" title="Sent">
      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
        <path d="M1 5l3 3 5-6" stroke="#6b7280" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    </span>
  );
}
...
export default function MessagesPage() {
  ...
  const openChat = async (chatUser) => {
    ...
    const existingRoom = rooms.find((r) => r.recipient_username === chatUser.username);
    if (existingRoom) {
  ...
  const openRoomChat = async (room) => {
    const chatUser = { id: room.recipient_id, username: room.recipient_username, avatar_url: room.recipient_avatar };
    setSelectedUser(chatUser);
    setMessages([]);
    setNewMsgIds(new Set());
    setLoadingMessages(true);
  ...
  const handleSend = async (e) => {
    ...
    setSending(true);
    try {
      const msg = await messageService.sendMessage(selectedUser.id, text.trim());
  ...
  const mutualNotInRooms = mutuals.filter((m) => !roomUsernames.has(m.username));

  return (
    <main className="flex-1 flex min-h-screen pt-8 pb-4 gap-0 max-w-4xl">
      {/* Left: Conversations list */}
      <div className="w-72 flex-shrink-0 border-r border-white/[0.06] pr-4 flex flex-col gap-1">
        ...
        {rooms.map((room) => (
          ...
          >
            <div className="relative">
              <Avatar username={room.recipient_username} avatarUrl={room.recipient_avatar} size="sm" />
              {/* Unread badge */}
              {room.unread_count > 0 && selectedUser?.username !== room.recipient_username && (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <p className={`text-sm font-semibold truncate ${room.unread_count > 0 ? "text-white" : "text-gray-300"}`}>
                  @{room.recipient_username}
                </p>
                {room.last_message_time && (
                  <p className={`text-[10px] flex-shrink-0 ml-1 ${room.unread_count > 0 ? "text-indigo-400" : "text-gray-600"}`}>
                    {timeAgo(room.last_message_time)}
                  </p>
                )}
              </div>
              {room.last_message && (
                <p className={`text-xs truncate ${room.unread_count > 0 ? "text-gray-300 font-medium" : "text-gray-500"}`}>
                  {room.last_message}
                </p>
              )}
            </div>
        ...
        {mutualNotInRooms.length > 0 && (
          <>
            <p className="text-[10px] text-gray-600 uppercase tracking-widest px-3 mt-3 mb-1">New Chat</p>
        ...
        {!selectedUser ? (
          ...
          <>
            ...
            <div className="flex items-center gap-3 pb-4 mb-4 border-b border-white/[0.06]">
              <button
                onClick={() => { setSelectedUser(null); setRoomId(null); setMessages([]); }}
              ...
              {loadingMessages ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm transition-all duration-500 ${
                        isMine
                      ...
                      <div className={`flex items-center gap-1 mt-0.5 px-1 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
                        <p className="text-[10px] text-gray-600">{formatTime(msg.created_at)}</p>
