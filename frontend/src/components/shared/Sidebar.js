...
export default function Sidebar() {
  ...
  return (
    <>
      <div className="md:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-[#0B0F19]/95 backdrop-blur">
        <Link href="/home" className="flex items-center gap-2">
          <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 text-white font-black text-lg shadow-lg">
            S
          </div>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-white/[0.06] bg-[#0B0F19]/95 backdrop-blur-xl">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-1 flex-col items-center justify-center rounded-xl px-2 py-2 text-[11px] font-medium transition-all ${
              ...
              >
                ...
                {item.badge > 0 && (
                  <span className="absolute top-1 right-3 bg-indigo-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full min-w-[16px] text-center">
